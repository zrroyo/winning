	...
	def __display (self, poll, painter, window, lineStart=1):
		...
		for k in keys:
			try:
				#print k, poll[k]
				dp = poll[k]
				output = '[%6s %s][Price: N/%g, B/%g, S/%g],[Vol: B/%d, S/%d, T/%d],[H/%g, L/%g],[%s]' % (dp.InstrumentID, ('↑' if dp.LastPrice > dp.PreClosePrice else '↓'), dp.LastPrice, dp.BidPrice1, dp.AskPrice1, dp.BidVolume1, dp.AskVolume1, dp.Volume, dp.HighestPrice, dp.LowestPrice, dp.UpdateTime)
