...
def ctpExecutionThreadStart(strategy, futCode, runCtrl, **extraArgs):
	...
	:param strategy: 交易策略
	:param futCode: 交易合约
	:param runCtrl: 运行时控制单元
	:param extraArgs: 其它参数
	"""
	strt1 = None
	...
	# 打开策略的CTP模式
	strt1.enableCTP(curDay, startTime, runCtrl, mdAgent, tdAgent,
			logPainter, logPainter.allocateLine())
...
def marketDataThreadStart(painter, window, mdAgent):
	"""
	行情显示线程入口
	:param painter: Painter描绘对象
	:param window: 描绘(子)窗口
	:param mdAgent: 行情数据代理接口
	"""
...
def ctpTradeCoreThreadStart(trade, tradeConfig, mdAgent, tdAgent, logPainter, logDir):
	"""
	CTP交易核心线程入口。从交易配置文件中读取参数并启动CTP交易。
	:param trade: 交易配置文件中的section名称
	:param tradeConfig: 交易配置文件读取接口
	:param mdAgent: 行情数据代理接口
	:param tdAgent: 交易服务器端代理
	:param logPainter: CTP日志管理接口
	:param logDir: 存储log的指定目录
	"""
	...
	# 填充通用交易属性
	comAttr = CommonAttrs(maxAddPos=int(tradeConfig.getMaxAddPos(trade)),
	...
	runCtrl1 = RunControl(False, thread.allocate_lock(), None, comAttr, False, startDate[0])
	runCtrl2 = RunControl(False, thread.allocate_lock(), None, comAttr, False, startDate[1])
	...
	# 初始化运行控制单元集
	runCtrlSet = RunCtrlSet(maxAllowedPos=int(tradeConfig.getAllowedPos(trade)), tickSrc=tickSrc)
	...
	if not isValidStrategy(strategy):
	...
	# 启动CTP
	emu = Emulate(strategy, runCtrlSet, instruments, ctpExecutionThreadStart, md=mdAgent, td=tdAgent,
		      log=logNameSuffix, logDir=logDir, logPainter=logPainter, startTime=startTime, database=database)
	emu.run()


def processOrder(details, mdBrokerid, mdInvestor, mdPasswd, mdServer, tdBrokerid, tdInvestor, tdPasswd, tdServer):
	"""
	下单处理
	:param details: 下单明细
	:param mdBrokerid:
...
def ctpOptionsHandler(options, args):
	...
	if options.mode == 'order':
		...
		processOrder(options.details, mdBrokerid, mdInvestor, mdPasswd, mdServer,
